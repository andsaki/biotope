...
開発タスクは、停止しやすい事前承認フローではなく、目的を絞った小さなブランチとPRで進めます。

1. `git status --short --branch` で作業ツリーを確認。
2. 目的が1つに絞れるブランチを作成。
3. 変更は小さく保ち、既存の構造と命名に合わせる。
4. 影響範囲に応じて `npx tsc --noEmit` / `npm run lint` / `npm run build` / `npm run smoke:browser` を実行。
5. PR本文はGit履歴から生成し、一時ファイルを作らない。
6. CIが通ったPRだけをマージ。
...
### パフォーマンスボトルネック調査と系統的な最適化

```
プロジェクト全体のパフォーマンスを体系的に改善してください。

ステップ1: ボトルネックの特定
Task tool (Explore エージェント) を使用してコードベース全体を分析：
- useFrame を使用しているコンポーネントをすべて見つける
- React.memo が使われていないコンポーネントを特定
- 毎フレーム新しいオブジェクトを作成している箇所を探す
- 重複したジオメトリ/マテリアル作成を探す
- インスタンス化されていないメッシュを特定

ステップ2: 優先度付けと計画
- 緊急度：高（即座に効果が見込める最適化）を優先
- 緊急度：中（安定性を保ちつつ効果的）を次に実施
- 低リスクで効果的な最適化に絞る
- 変更方針と検証観点を短く整理

ステップ3: 最適化の実施
以下のパターンを適用：
1. Three.js オブジェクトの再利用
   - Color オブジェクトを useRef で保持し .set() で再利用
   - Vector3 など他のオブジェクトも同様に
2. ジオメトリ・マテリアルの共有
   - 同じジオメトリを複数回作成している箇所を useMemo で共有
3. React.memo によるメモ化
   - Props が変わらない限り再レンダリングしないようにする
4. useFrame 内での setState 削除
   - ref で直接更新してReactの再レンダリングを回避

ステップ4: 検証
- TypeScript 型チェック（npx tsc --noEmit）
- ビルドテスト（npm run build）
- パフォーマンスモニターで FPS 確認
- 各変更は独立させて、問題があれば個別にロールバック可能に
...
参考実装:
- tmp/008_performance-improvement/ （実装例とパターン）
- docs/ai-guide/architecture.md の「パターン8」
- README.md の「パフォーマンス最適化」セクション
```
...
- 夜なのに「夕暮れ」のメッセージが表示される
- 時間帯が変わっても同じ内容のメッセージが表示され続ける
- 17時に生成された「夕方」のメッセージが21時以降も表示される

**原因**:

1. **キャッシュキーに時間帯が含まれていない**: `2026-01-26`のように日付のみでキャッシュしているため、時間帯が変わっても同じメッセージが返される
2. **日次キャッシュの仕様**: 1日1回の更新を想定しており、日中の時間帯変化に対応していない

**解決策**:

#### 解決策1: 時間帯判定関数を追加（日本語・英語）
...
#### 解決策2: キャッシュキーに時間帯を含める

```typescript
// functions/api/daily-message.ts - onRequest関数内
...
**重要なポイント**:
- キャッシュキーは英語表記 (`morning`, `afternoon`, `evening`, `night`)
- ユーザー表示は日本語表記 (`朝`, `昼`, `夕方`, `夜`)
- 時間帯が変わる（5時、11時、17時、21時）とキャッシュミスになり、新しいメッセージが生成される
...
| 時刻 | キャッシュキー | 説明 |
|-----|-------------|------|
| 5:00-10:59 | `2026-01-26-morning` | 朝のメッセージ |
| 11:00-16:59 | `2026-01-26-afternoon` | 昼のメッセージ |
| 17:00-20:59 | `2026-01-26-evening` | 夕方のメッセージ |
| 21:00-4:59 | `2026-01-26-night` | 夜のメッセージ |

**重要な注意点**:

- 時間帯判定の基準は `src/utils/time.ts` の `getTimeOfDay()` と同じにする
- 日本時間（JST）で判定することを確認（`toLocaleString('en-US', { timeZone: 'Asia/Tokyo' })`）
- キャッシュは**日付+時間帯**単位で管理（例: `2026-01-26-night`）
- 時間帯が変わる境界（5時、11時、17時、21時）で新しいメッセージが生成される
