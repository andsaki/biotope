...
このドキュメントは、水辺の四季プロジェクト固有の技術用語とドメイン用語を定義します。
...
### トークンベースデザイン
デザインシステムの手法の一つ。色、スペーシング、タイポグラフィなどの値を定数（トークン）として管理し、一貫性を保つ。水辺の四季では`src/constants/`配下で厳密に管理。
...
const Loader = () => (
  <div className={styles.container}>
    <h1 className={styles.title}>水辺の四季</h1>
  </div>
);
```
...
#### useThrottledFrame
水辺の四季プロジェクトで実装されたカスタムフック。`useFrame`の実行頻度を制限（例: 30fps）し、重い処理のパフォーマンスを改善。
...
#### GLTF / GLB
3Dモデルのファイル形式。GLTFはJSON + 外部ファイル、GLBはバイナリ形式（単一ファイル）。水辺の四季では主にGLBを使用。
...
#### Cloudflare Pages
静的サイトホスティングサービス。GitHubと連携し、コミット時に自動デプロイ。水辺の四季のフロントエンドをホスト。

#### Cloudflare R2
オブジェクトストレージサービス。S3互換。水辺の四季では3Dモデル（.glbファイル）の配信に使用。
...
#### Cloudflare KV
Key-Valueストレージ。エッジで高速にアクセス可能。水辺の四季では日替わりメッセージのキャッシュに使用。
...
Cloudflareのエッジで動作するAI推論サービス。水辺の四季では日替わりメッセージの生成に使用。
...
#### Gemini API
GoogleのAI APIサービス。水辺の四季では日替わりメッセージの生成に使用。
...
1秒あたりのフレーム数。水辺の四季では60FPSを目標とし、常にパフォーマンスモニタリングを実施。
...
半透明のガラスのような質感を持つUIデザイン手法。水辺の四季では水のコンテキストに合わせた主要なデザイン言語として採用。
...
### デザインアンチパターン
水辺の四季プロジェクトで**絶対に使用してはいけない**デザインパターン。
...
**用途**: 深い瞑想的コンテンツ（水辺の四季の日替わりメッセージには不向き）
